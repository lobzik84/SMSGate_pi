...
/**
 * This class encapsulates the basic characteristics of an SMS message. A
 * message is further subclassed to an "Incoming" message and an "Outgoing"
 * message.
 * <br><br>
 * This class is <strong>never</strong> used directly. Please use one of its
 * descendants.
 *
...
public class CMessage {
     ...
     * @param	originator	the originator's number. Applicable only for incoming
     * messages.
     * @param	recipient	the recipient's number. Applicable only for outgoing
     * messages.
     ...
     * <br><br>Notes:<br>
     * <ul>
     * <li>Phone numbers are represented in their international format (e.g.
     * +306974... for Greece).</li>
    ...
    /**
     * Returns the type of the message. Type is either incoming or outgoing, as
     * denoted by the class' static values INCOMING and OUTGOING.
     *
    ...
    /**
     * Returns the date of the message. For incoming messages, this is the sent
     * date. For outgoing messages, this is the creation date.
     *
    ...
    private String getPDU(String smscNumber, String text) {
        ...
        if (udh != null) {
            pdu = pdu + "51";//PDU-Type
            pdu = pdu + String.format("%02X", udh[3]);//message reference
        } else 
        {
            pdu = pdu + "11";//PDU-Type
            pdu = pdu + "00";//message reference
        }
        ...
        pdu = pdu + str2 + str1;
        pdu = pdu + "00";//TP- PID
        switch (getMessageEncoding()) { //TP- DCS 
        ...
        if (udh != null) {
            ...
            encodedText = sb.toString() + encodedText;
        }
        
        // Encode the length of the data, TP-UDL 
